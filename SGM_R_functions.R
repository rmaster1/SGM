...
#Robin Ristl
#Center for Medical Informatics, Statistics, and Intelligent Systems
...
sgm_fun<-function(y,time,id,jackknife=FALSE,laplace=TRUE,M=8,showplot=TRUE) {
	...
	#Achtung: indi_lambda kann auch negativ sein, was im finalen Modell nicht erlaubt ist.
	...
	likelihood_i<-function(theta,log_nu,log_k,pat,tol=10^-6,maxiter=40) {
		...
		#integrate braucht als Input eine Funktion, die einen Vektor übernimmt und ausgibt
		...
		#c(Int$value,log(Int$value),length(y))
		...
		f0<-function(x) sum( -(y-(exp(x)-exp(2*x)*k^2/2)*z)^2 /(2*k^2*exp(2*x)*z) )  -(x-theta)^2/(2*nu2)-x*N
            #                    -(exp(-2*x)*(k^2*z*exp(2*x)-2*y)*(k^2*z*exp(2*x)-2*z*exp(x)+2*y))/(4*k^2*z)
		f1<-function(x) sum( -(exp(-2*x)*(k^2*z*exp(2*x)-2*y)*(k^2*z*exp(2*x)-2*z*exp(x)+2*y))/(4*k^2*z) )  -(x-theta)/nu2-N
		#			   -(exp(-2*x)*(k^4*z^2*exp(4*x)-k^2*z^2*exp(3*x)-2*y*z*exp(x)+4*y^2))/(2*k^2*z)
		...
		#x0 finden, als argmax des Integranden über beta
		#bei großen Werten für nu wird die Kurve sehr flach und das
		#numerische Finden des Maximums mit dem Newton Algorithmus kann schiefgehen. (f2 kann schwach positiv werden, was eigentlich nicht passieren sollte).
		#daher mit optimize Maximum suchen
		...
		#while(!stop) {
		#	iter<-iter+1
			#print(paste(pat,x0,x0_alt,f1(x0_alt),f2(x0_alt),iter,0,theta,log_nu,log_k))
		#	x0<-x0_alt-f1(x0_alt)/f2(x0_alt)
			#print(paste(pat,x0,x0_alt,f1(x0_alt),f2(x0_alt),iter,1,theta,log_nu,log_k))
			#flush.console()
		#	if(abs(x0-x0_alt)<tol | iter==maxiter) stop<-TRUE
		#	x0_alt<-x0
		#}
		
		#if(iter==maxiter) warning(paste("Maximale Iterationszahl erreicht, pat",pat))
				
		logIntLaplace<- -N/2 *log(2*pi)  -0.5*sum(log(z)) -0.5*log(2*pi)+ 0.5*log(2*pi) -N*log(k) -log(nu) -0.5*log(-f2(x0))+f0(x0)
		#propLogLik<- -N*log(k) -log(nu) -0.5*log(-f2(x0))+f0(x0)

		return(logIntLaplace) #besser die echte Loglikelihood, weil wir dann Likelihoodratio Tests machen können,
					#oder ist es egal, weil der Rest sich wegkürzt?
		#return(propLogLik)
		}
	}
...
faltfun_update<-function(q,zeit,y,t,theta_est,nu_est,k_est,nn=300,M=6) {
	...
	#plot(s,f_s,type="l")
	#bed_dichte_nenner<-sum(f_s*w)
	#plot(s,f_s,type="l")
	#integrate(bed_dichte_zaehler_fun,lower=LOW,upper=UP) #hat numerische Probleme
	#bed_dichte_nenner
	#abline(v=x0)
	#abline(v=theta)
	#abline(v=c(LOW,UP))
